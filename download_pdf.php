    ...
    function Header()
{
    ...
    $this->Cell(0, 10, 'Sales Report', 0, 0, 'L'); // Title aligned to the right
...
if (isset($_GET['start_date']) && isset($_GET['end_date'])) {
    ...
    $query = "SELECT * FROM invoice WHERE invoice_date BETWEEN '$start_date' AND '$end_date'";
    ...
    $pdf->Cell(40, 10, 'PytUptd-Date', 1, 1, 'C');
    ...
    foreach ($query_run as $row) {
        ...
        $pdf->Cell(20, 10, $row['invoice_id'], 1, 0, 'C');
        $pdf->Cell(30, 10, $row['invoice_date'], 1, 0, 'C');
        $pdf->Cell(25, 10, $row['total_cost'], 1, 0, 'C');
        $pdf->Cell(30, 10, $row['amt_paid'], 1, 0, 'C');
        $pdf->Cell(25, 10, $row['balance'], 1, 0, 'C');
        $pdf->Cell(40, 10, $row['paymentupdate_date'], 1, 1, 'C');
    }

    // Summary
    $pdf->SetFont('Arial', 'B', 12);
    $pdf->Cell(40, 10, '', 0, 1);
    $pdf->Cell(40, 10, 'Summary', 1);
    $pdf->Cell(40, 10, '', 0, 1);
    $pdf->SetFont('Arial', '', 12);
